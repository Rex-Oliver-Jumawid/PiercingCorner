...
export function StudioConfigurationView({ configuration, editor, setEditor }: { configuration: StudioConfiguration; editor: StudioEditor | null; setEditor: (value: StudioEditor | null) => void }) {
  ...
  return <>
    <section id="studio-hours" tabIndex={-1} className="studio-panel">
      ...
      {temporarySchedules.active ? <div className="studio-active-temporary"><div><span className="studio-source-badge temporary">Temporary</span><strong>{formatStudioDate(temporarySchedules.active.starts_on)} – {formatStudioDate(temporarySchedules.active.ends_on)}</strong><small>Recurring schedule resumes {formatStudioDate(addCalendarDays(temporarySchedules.active.ends_on, 1), false)}.</small></div><button className="studio-row-edit" type="button" aria-label={`Edit temporary Studio schedule from ${formatStudioDate(temporarySchedules.active.starts_on)} to ${formatStudioDate(temporarySchedules.active.ends_on)}`} onClick={() => setEditor({ mode: 'configure-hours', schedule: temporarySchedules.active! })}>Edit</button></div> : null}
      {visibleTemporarySchedules.filter((schedule) => schedule.id !== temporarySchedules.active?.id).length ? <div className="studio-temporary-list"><p>Upcoming temporary schedules</p>{visibleTemporarySchedules.filter((schedule) => schedule.id !== temporarySchedules.active?.id).map((schedule) => <div className="studio-temporary-row" key={schedule.id}><div><strong>{formatStudioDate(schedule.starts_on)} – {formatStudioDate(schedule.ends_on)}</strong><small>Recurring schedule resumes {formatStudioDate(addCalendarDays(schedule.ends_on, 1), false)}.</small></div><button className="studio-row-edit" type="button" aria-label={`Edit temporary Studio schedule from ${formatStudioDate(schedule.starts_on)} to ${formatStudioDate(schedule.ends_on)}`} onClick={() => setEditor({ mode: 'configure-hours', schedule })}>Edit</button></div>)}</div> : null}
    ...
    <section className="studio-panel"><header className="studio-panel-head"><div><h3>Piercer Availability</h3><p>Recurring weekly availability with date-bounded overrides.</p></div>{availabilityProfile ? <button className="catalog-button primary" type="button" onClick={() => setEditor({ mode: 'configure-piercer-schedule' })}>Configure schedule</button> : null}</header>
      {availabilityProfile ? <><div className="studio-tabs">{configuration.profiles.map((item) => <button type="button" className={availabilityProfile.id === item.id ? 'active' : ''} key={item.id} onClick={() => setSelectedAvailabilityId(item.id)}>{item.display_name}{item.active ? '' : ' · Inactive'}</button>)}</div><p className="studio-selected">Selected: <strong>{availabilityProfile.display_name}</strong>{availabilityProfile.active ? '' : ' · Inactive profile'}</p>
        {piercerTemporarySchedules.active ? <div className="studio-active-temporary"><div><span className="studio-source-badge temporary">Temporary</span><strong>{formatStudioDate(piercerTemporarySchedules.active.starts_on)} – {formatStudioDate(piercerTemporarySchedules.active.ends_on)}</strong><small>Recurring schedule resumes {formatStudioDate(addCalendarDays(piercerTemporarySchedules.active.ends_on, 1), false)}.</small></div><button className="studio-row-edit" type="button" aria-label={`Edit temporary schedule for ${availabilityProfile.display_name} from ${formatStudioDate(piercerTemporarySchedules.active.starts_on)} to ${formatStudioDate(piercerTemporarySchedules.active.ends_on)}`} onClick={() => setEditor({ mode: 'configure-piercer-schedule', schedule: piercerTemporarySchedules.active! })}>Edit</button></div> : null}
        {piercerTemporarySchedules.upcoming.length ? <div className="studio-temporary-list"><p>Upcoming temporary schedules</p>{piercerTemporarySchedules.upcoming.map((schedule) => <div className="studio-temporary-row" key={schedule.id}><div><strong>{formatStudioDate(schedule.starts_on)} – {formatStudioDate(schedule.ends_on)}</strong><small>Recurring schedule resumes {formatStudioDate(addCalendarDays(schedule.ends_on, 1), false)}.</small></div><button className="studio-row-edit" type="button" aria-label={`Edit temporary schedule for ${availabilityProfile.display_name} from ${formatStudioDate(schedule.starts_on)} to ${formatStudioDate(schedule.ends_on)}`} onClick={() => setEditor({ mode: 'configure-piercer-schedule', schedule })}>Edit</button></div>)}</div> : null}
        <div className="studio-recurring-label"><span className="studio-source-badge recurring">Recurring</span><span>Individual edits below only change the recurring weekday.</span><button className="studio-row-edit" type="button" aria-label={`Edit recurring schedule for ${availabilityProfile.display_name}`} onClick={() => setEditor({ mode: 'configure-piercer-schedule' })}>Edit</button></div>
